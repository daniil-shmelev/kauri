...
def _antipode(t):
    ...
    return (
        out.singleton_reduced().simplify()
    )  # Single node tree is the unit, so can apply .singleton_reduced() here
        ...
        for p in itertools.product(*subtree_coproduct_trees):
            ...
            for edge, t_ in zip(edges, p):
                if t_.list_repr is None:
                    continue
                rep += (
                    t_.list_repr[:-1] if edge else [t_.list_repr]
                )  # Add to tree, contracting the edge if necessary
