...
class LabelledReprComparison:
    ...
    def __lt__(self, other):
        ...
        return (
            self.unlabelled_rep > other.unlabelled_rep
        )  # The ">" here is not a typo, we want the reverse order.
...
def _sorted_list_repr(rep):
    ...
    *children, label = rep
    sorted_children = tuple(
        sorted((_sorted_list_repr(child) for child in children), key=LabelledReprComparison)
    )
